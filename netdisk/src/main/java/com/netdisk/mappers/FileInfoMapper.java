...
public interface FileInfoMapper<T, P> extends BaseMapper<T, P> {
    ...
    T selectByFileId(String fileId);

    List<String> selectAllSubFileIds(@Param("fileId") String fileId, @Param("userId") String userId);

    int countCollectPublicFileList(P query);

    List<T> selectCollectPublicFileList(P query);
