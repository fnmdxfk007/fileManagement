...
public class FileInfoServiceImpl implements FileInfoService {
     ...
     * 分页查询公开文件
     */
    public PaginationResultVO<FileInfo> loadPublicFileList(FileInfoQuery query) {
        query.setPermission(1); // 公开文件
        query.setAccessType(0); // 只读和下载权限
        query.setDelFlag(FileDelFlagEnums.USING.getFlag()); // 仅查询正常文件
        query.setOrderBy(query.getOrderBy() != null ? query.getOrderBy() : "last_update_time desc"); // 排序条件
     ...
     */
    @Override
    public PaginationResultVO<FileInfo> loadCollectPublicFileList(FileInfoQuery query) {
        int count = fileInfoMapper.countCollectPublicFileList(query);
        int pageSize = query.getPageSize() == null ? PageSize.SIZE15.getSize() : query.getPageSize();
        SimplePage page = new SimplePage(query.getPageNo(), count, pageSize);
        query.setSimplePage(page);
        List<FileInfo> list = fileInfoMapper.selectCollectPublicFileList(query);
    ...
    public void setIsPublic(String fileId, String userId, Integer permission, Integer accessType) {
        if (StringUtils.isBlank(fileId)) {
            throw new IllegalArgumentException("文件ID不能为空");
        }
        if (permission == null || accessType == null) {
        ...
        for (String fid : fileIdArr) {
            ...
            FileInfo fileInfo = fileInfoMapper.selectByFileIdAndUserId(fid, userId);
            if (fileInfo == null) {
                throw new BusinessException("文件不存在或无权操作：" + fid);
            }
            // 递归更新自身和所有子节点
     ...
     * 递归更新自身和所有子节点的权限
     */
    private void updateAllPermissionRecursively(String fileId, String userId, Integer permission, Integer accessType, Date updateTime) {
        // 1. 更新自身
        FileInfo updateBean = new FileInfo();
        updateBean.setPermission(permission);
        updateBean.setAccessType(accessType);
        updateBean.setLastUpdateTime(updateTime);
        fileInfoMapper.updateByFileIdAndUserId(updateBean, fileId, userId);

        // 2. 查询所有直接子节点
        FileInfoQuery query = new FileInfoQuery();
        query.setUserId(userId);
        query.setFilePid(fileId);
        query.setDelFlag(2); // 只查正常文件
        List<FileInfo> children = fileInfoMapper.selectList(query);

        System.out.println("递归fileId=" + fileId + "，children数量=" + children.size());

        // 3. 递归更新
        for (FileInfo child : children) {
            updateAllPermissionRecursively(child.getFileId(), userId, permission, accessType, updateTime);
        }
    }
    /**
     * 更新文件简介
    ...
    public void updateFileDescription(String fileId, String userId, String description) {
        // 参数校验: 确保 fileId 和 userId 非空
        if (StringUtils.isAnyBlank(fileId, userId)) {
            throw new BusinessException(ResponseCodeEnum.CODE_600); // 统一错误码
        }
        // 构造更新对象
        FileInfo update = new FileInfo();
        update.setDescription(StringUtils.trimToEmpty(description)); // 去除多余空格并允许清空简介
        update.setLastUpdateTime(new Date()); // 更新时间
        // 数据库更新操作
        int rows = fileInfoMapper.updateByFileIdAndUserId(update, fileId, userId);
        // 如果更新失败，抛出异常
        if (rows == 0) {
            throw new BusinessException("文件不存在或操作权限不足");
        }
    }
    /**
     * 新增
        ...
        try {
            ...
            if (chunkIndex == 0) {
                ...
                if (!dbFileList.isEmpty()) {
                    ...
                    dbFile.setPermission(permission != null ? permission : 0); // 默认权限类别为个人
                    dbFile.setAccessType(accessType);
            ...
            fileInfo.setPermission(permission != null ? permission : 0); // 默认权限类别为个人
    ...
    @Override
    @Transactional(rollbackFor = Exception.class)
    public FileInfo newFolder(String filePid, String userId, String folderName,Integer permission,Integer accessType) {
        ...
        fileInfo.setDelFlag(FileDelFlagEnums.USING.getFlag());
        fileInfo.setPermission(permission != null ? permission : 0);
        fileInfo.setAccessType(accessType);
